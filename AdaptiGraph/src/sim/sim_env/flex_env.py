...
class FlexEnv(gym.Env):
    ...
    def reset_robot(self, jointPositions = np.zeros(14)):  
        index = 0
        for j in range(9):
    ...
    def add_robot(self):
        if self.obj in ['granular']:
            ...
            robot_base_orn = [0, 0, 0, 1]
            self.robotId = pyflex.loadURDF(self.flex_robot_helper, 'sim/assets/fr3/fr3_nogripper.urdf', 
                                            robot_base_pos, robot_base_orn, globalScaling=10.0) 
            # self.rest_joints = np.zeros(8)
            # self.rest_joints = [0, 0, 0, 0.5 * math.pi, 0, -math.pi * 0.5 * 0.66, 0,0]
            rest_pose = [
                0,
                0.0,      # Joint 1
                -0.785,   # Joint 2 (slightly bent)
                0.0,      # Joint 3
                -2.355,   # Joint 4 (deep bend)
                0.0,      # Joint 5
                1.57,     # Joint 6 (wrist upright)
                0.785,    # Joint 7 (gripper aligned)
                0.04,     # Finger 1 (slightly open)
                0.04      # Finger 2 (slightly open)
            
            ]
            self.rest_joints = np.array(rest_pose)
        ...
        for j in range(len(self.camPos_list)):
            ...
            if not saved_particles:
                ...
                if self.gripper:
                    ...
                    eef_states[1] = robot_shape_states[10] # right finger
                else:
    ...
    def robot_close_gripper(self, close, jointPoses=None):
        for j in range(9, self.num_joints):
    ...
    def reset(self, save_data=False):
        ...
        for i in range(self.num_joints):
            ...
            jointType = info[2]
            if (jointType == p.JOINT_PRISMATIC or jointType == p.JOINT_REVOLUTE):
                self.ll[dof_idx] = info[8]
                self.up[dof_idx] = info[9]
                self.jointDamping[dof_idx] = info[6]
                dof_idx += 1
        print(f"Lower limits: {self.ll}")
        print(f"Upper limits: {self.up}")
        self.reset_robot()
    ...
    def step(self, action, save_data=False, data=None):
        ...
        for i_p in range(len(way_points)-1):
            ...
            for i in range(steps):
                ...
                print(f"info: {jointPoses}", end="\r", flush=True)  # Overwrite with new info
                pyflex.step()
                ...
                if self.gripper and i_p >= 1:
                    ...
                    if i_p == 1:
                        ...
                        for j in range(close_steps):
                            ...
                            left_finger_pos, right_finger_pos = robot_shape_states[9][:3], robot_shape_states[10][:3]
                    ...
                    left_finger_pos[1], right_finger_pos[1] = left_finger_pos[1] - finger_y, right_finger_pos[1] - finger_y
                    new_finger_pos = (left_finger_pos + right_finger_pos) / 2
                    # print('new_finger_pos:', new_finger_pos)
